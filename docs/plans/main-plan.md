...
- 黒本の全23章を「構造的」に理解し、Codespaces上で実機確認する。
...
以下のツールは学習対象ではなく、Codespaces 環境の制約を吸収するために導入しています。
各章の README では原則として説明しません。

| ツール | 用途 | なぜ必要か |
| :--- | :--- | :--- |
| nginx | EM Express ブラウザアクセス用リバースプロキシ | Oracle EM Express は HTTPS 必須かつ `.js` ファイルを `text/plain` で返す既知の挙動がある。Codespaces のポート転送（HTTP）からの接続を nginx で受け、Oracle tcps（5500）へ転送しつつ Content-Type を修正する |

### SQL Developer 接続（ローカル）

ローカルにインストールした Oracle SQL Developer から Codespaces の Oracle XE に接続できます。
**Oracle Instant Client は不要**です。SQL Developer 内蔵の Thin JDBC ドライバが TCPS（TLS）をサポートしています。

#### 初回セットアップ（Codespaces 再ビルド後も同様）
...
1. TCPS リスナー用の自己署名サーバー証明書を生成する
2. `listener.ora` に TCPS ポート 2484 を追加してリスナーを再起動する
3. SQL Developer 用のクライアント Wallet（`sql-developer-wallet/wallet.zip`）を生成する

#### wallet.zip のダウンロード

VS Code のエクスプローラーで `sql-developer-wallet/wallet.zip` を右クリック → 「ダウンロード」
...
> **前提**: VS Code で Codespaces に接続した状態で使用してください。VS Code がポート 2484 をローカルに転送します。
> **Codespaces 再ビルド時**: `generate-wallet.sh` を再実行して新しい `wallet.zip` を取得してください。
...
カリキュラム制作・検証には以下のカスタムスラッシュコマンドを使用する。

| コマンド | 役割 | 主な使用タイミング |
| :--- | :--- | :--- |
| `/verify-db` | 対象章の SQL・DBA コマンドを実機で実行して正確性を検証する | README.md の新規作成後、内容変更後 |
| `/draft-chapter` | 章の `README.md` を作成し品質を評価する | 新章の執筆開始時 |
| `/plan` | カリキュラムの章構成・学習目標・実習内容を計画・更新する | モジュール設計時、学習目標の見直し時 |

**使用例:**

```text
/verify-db module-01-architecture の全ステップを実機で検証してください
/draft-chapter module-02-instance の README.md を作成してください
```
